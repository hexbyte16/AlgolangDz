...
export const Docs: React.FC<DocsProps> = ({ onBack, isDarkMode }) => {
  ...
  return (
    <div className={`flex h-full w-full ${isDarkMode ? 'bg-[#0a1f13] text-slate-200' : 'bg-white text-slate-800'}`}>
      ...
      {/* Sidebar Navigation */}
      <div className={`w-64 flex-shrink-0 flex flex-col border-r h-full overflow-y-auto custom-scrollbar ${isDarkMode ? 'border-emerald-900/30 bg-[#0f281a]' : 'border-slate-100 bg-slate-50'}`}>
                ...
                <div key={category.title}>
                    ...
                    <div className="space-y-1">
                        {category.pages.map(page => (
                            ...
                            >
                                {page.title}
                                {activePageId === page.id && <ChevronRight size={14} />}
                            </button>
                        ))}
                    </div>
                </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar scroll-smooth">
         <div className="max-w-4xl mx-auto px-8 py-12">
            ...
            <div className="mt-20 pt-10 border-t border-inherit flex justify-between">
                {getPrevPage(activePageId) && (
                    <button onClick={() => setActivePageId(getPrevPage(activePageId)!.id)} className="text-left group">
                        <div className={`text-xs uppercase mb-1 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Previous</div>
                        <div className={`text-lg font-medium flex items-center gap-2 group-hover:text-emerald-500 transition-colors`}>
                           <ArrowLeft size={18} /> {getPrevPage(activePageId)!.title}
                        </div>
                    </button>
                )}
                 {getNextPage(activePageId) && (
                    <button onClick={() => setActivePageId(getNextPage(activePageId)!.id)} className="text-right group ml-auto">
                        <div className={`text-xs uppercase mb-1 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Next</div>
                        <div className={`text-lg font-medium flex items-center gap-2 group-hover:text-emerald-500 transition-colors`}>
                            {getNextPage(activePageId)!.title} <ChevronRight size={18} />
                        </div>
                    </button>
                )}
            </div>
...
const BlockRenderer: React.FC<{ block: DocBlock, isDarkMode: boolean }> = ({ block, isDarkMode }) => {
    switch (block.type) {
        case 'text':
            return <p className={`leading-7 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{block.value}</p>;
        ...
        case 'note':
            ...
            return (
                <div className={`p-4 rounded-lg border flex gap-3 my-6 ${colorClass}`}>
                    ...
                    <div className="text-sm leading-6 font-medium opacity-90">{block.value}</div>
                </div>
            );
...
const flatPages = DOCS_DATA.flatMap(c => c.pages);
const getPrevPage = (id: string) => {
    const idx = flatPages.findIndex(p => p.id === id);
    return idx > 0 ? flatPages[idx - 1] : null;
};
const getNextPage = (id: string) => {
    const idx = flatPages.findIndex(p => p.id === id);
    return idx < flatPages.length - 1 ? flatPages[idx + 1] : null;
};